...
# randomly split train/val/test datasets
...
def gen_splits(
    ...
    #     val_idx = unknown_idx
    # else:
    val_idx = exclude_idx(all_idx, [train_idx, stopping_idx])
...
def gen_splits2(split_idx,labels, train_rate=0.9):
    train = np.array(split_idx['train'])
    val_idx = np.array(split_idx['test'])
    temp = labels[split_idx['train']]
    _, counts = np.unique(temp, return_counts=True)
    ntrain_per_class = counts[0]
    train_per_class = int(ntrain_per_class * train_rate)
    nstopping =int(train.shape[0]*0.1)
    train_idx, stopping_idx = train_stopping_split(train, labels[split_idx['train']], train_per_class, nstopping)
    return train_idx, stopping_idx, val_idx